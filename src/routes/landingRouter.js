...
router.get("/mass/:mass", async(req, res) => {
    res.send(await Landing.find({mass: req.params.mass}).select("name mass recclass geolocation"))
})

router.get("/class/:recclass", async(req, res) => {
    const recclass = req.params.recclass.toUpperCase()
    res.send(await Landing.find({recclass: recclass}).select("name mass recclass geolocation"))
})
